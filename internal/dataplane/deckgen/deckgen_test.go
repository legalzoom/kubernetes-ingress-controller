...
func Benchmark_gojson(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		bb, err := GenerateSHA_gojson(&targetContent)
		require.NoError(b, err)
		_ = bb
	}
}

func Benchmark_gob_crc32(b *testing.B) {
	var targetContent file.Content
	require.NoError(b, json.Unmarshal([]byte(configJSON), &targetContent))
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		bb, err := GenerateSHA_gob_crc32(&targetContent)
		require.NoError(b, err)
		_ = bb
	}
}

func Benchmark_gob_sha256(b *testing.B) {
	var targetContent file.Content
	require.NoError(b, json.Unmarshal([]byte(configJSON), &targetContent))
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		bb, err := GenerateSHA_gob_sha256(&targetContent)
		require.NoError(b, err)
		_ = bb
	}
}

func Benchmark_gojson_cache(b *testing.B) {
	var targetContent file.Content
	require.NoError(b, json.Unmarshal([]byte(configJSON), &targetContent))
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		bb, err := GenerateSHA_gojson_cache(&targetContent)
		require.NoError(b, err)
		_ = bb
	}
}

func Benchmark_gojson_nopool(b *testing.B) {
	var targetContent file.Content
	require.NoError(b, json.Unmarshal([]byte(configJSON), &targetContent))
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		bb, err := GenerateSHA_gojson_nopool(&targetContent)
		require.NoError(b, err)
		_ = bb
	}
}

func Benchmark_gojson_pool(b *testing.B) {
	var targetContent file.Content
	require.NoError(b, json.Unmarshal([]byte(configJSON), &targetContent))
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		bb, err := GenerateSHA_gojson_pool(&targetContent)
		require.NoError(b, err)
		_ = bb
	}
}
