...
// StoreReader is the interface that wraps the required methods to gather information about the Kubernetes objects
// that have been added by controllers to the store.
...
// Writer is the interface that wraps the required methods to add/update, and delete objects from the store.
...
// Store implements StoreReader and can be used to list Ingress, Services
...
func New(cs CacheStores, ingressClass string, logger logr.Logger) StoreReader {
	return Store{
		stores:                cs,
...
func (s Store) UpdateObject(obj client.Object) error {
...
func (s Store) DeleteObject(obj client.Object) error {
...
func (s Store) ObjectExists(obj client.Object) (bool, error) {
	_, exists, err := s.stores.Get(obj)
	return exists, err
}
